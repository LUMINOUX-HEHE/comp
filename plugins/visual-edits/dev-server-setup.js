...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // (Optional) add your own domains here, for example:
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
              ...
              changesAtLine.forEach((change) => {
                ...
                if (
                  ...
                  if (parentElementPath && parentElementPath.isJSXElement()) {
                    ...
                    const { code: oldCode } = generate(oldContentAST, {}, "");
                    const oldContent = oldCode
                      .replace(/^<>/, "")
                      .replace(/<\/>$/, "")
                      .trim();
          ...
          try {
            ...
            execSync(
              `git -c user.name="visual-edit" -c user.email="dev-server@example.com" add "${targetFile}"`,
            );
            execSync(
              `git -c user.name="visual-edit" -c user.email="dev-server@example.com" commit -m "visual_edit_${timestamp}"`,
            );
          } catch (gitError) {
